...
d3.csv("RSQSubmissions.csv", function (data) {
    console.log("==============>>>>>>>>>>>",data);
    ...
    yearData.forEach((c, idx) => {
        ...
        var nodes = d3.range(total).map(function (d, i) {
            return {
                size: size,
                y: height - ((i%rows) * size + 5),
                x: prevEnd + barMargin + Math.floor((i/rows)) * size,
                //y: height - (Math.floor((i / rows)) * size),
...
function createUnitVis(currYear) {
    ...
    units.transition()
        .duration(500)
        .style("fill", function (d) { return colorScale(2011) })
